...
function CompanionBody({speaker,mood,talking}:{speaker:Speaker;mood:Mood;talking:boolean}){
  const src=speaker==='Myria'?BASE_PATH+'/myria-profile.jpg':BASE_PATH+'/aetherfall-icon.png';
  return <figure className={styles.bodyAvatar} data-speaker={speaker} data-mood={mood} data-talking={talking}>
    <div className={styles.staticPortrait}>
      <img src={src} alt={speaker+' AI companion avatar'} />
      <span className={styles.portraitStatus}>{talking?'speaking':mood}</span>
    </div>
    <figcaption><strong>{speaker}</strong><span>{speaker==='Myria'?'Creative & Support':'Strategy & Structure'}</span></figcaption>
  </figure>;
}
