...
type SpeechRecognitionLike={continuous:boolean;interimResults:boolean;lang:string;start:()=>void;stop:()=>void;abort:()=>void;onresult:((event:SpeechEventLike)=>void)|null;onend:(()=>void)|null;onerror:((event:{error?:string})=>void)|null};
type SpeechRecognitionCtor=new()=>SpeechRecognitionLike;
const MYRIA_WELCOME="I'm Myria, your persistent RPG writing companion. I remember your cloud project across the website and Android app, help build any kind of book, and can work through Plan → Act → Review → Revise → Self-review before I answer.";
const MYRIA_PROCESS="Work through this bounded creative loop before answering: PLAN a concise approach; ACT by producing the requested work; REVIEW it against the user's request and project memory; REVISE weak points; then SELF-REVIEW for continuity, usefulness, and contradictions. Repeat review/revision only when it materially improves the result, then stop. Do not expose hidden chain-of-thought; return only useful conclusions, drafts, checklists, decisions, and the polished final result.";
function speechCtor():SpeechRecognitionCtor|null{if(typeof window==='undefined')return null;const w=window as Window&{SpeechRecognition?:SpeechRecognitionCtor;webkitSpeechRecognition?:SpeechRecognitionCtor};return w.SpeechRecognition??w.webkitSpeechRecognition??null;}
...
export default function RedboundApp(){
  ...
  const [chat,setChat]=useState(''); const [myriaStatus,setMyriaStatus]=useState<MyriaStatus>('idle'); const [muted,setMuted]=useState(false); const [goals,setGoals]=useState<MyriaGoal[]>([]); const [focusMode,setFocusMode]=useState(false); const [voiceAlwaysOn,setVoiceAlwaysOn]=useState(false); const [voiceSupported,setVoiceSupported]=useState(false); const [lastHeard,setLastHeard]=useState('');
  ...
  const saveTimers=useRef<Record<string,ReturnType<typeof setTimeout>>>({}); const audioRef=useRef<HTMLAudioElement|null>(null); const voiceAbort=useRef<AbortController|null>(null); const speechQueue=useRef<string[]>([]); const speakingRef=useRef(false); const recognitionRef=useRef<SpeechRecognitionLike|null>(null); const keepListeningRef=useRef(false); const recognitionRestart=useRef<ReturnType<typeof setTimeout>|null>(null);
  ...
  useEffect(()=>{setVoiceSupported(Boolean(speechCtor()));return()=>{keepListeningRef.current=false;if(recognitionRestart.current)clearTimeout(recognitionRestart.current);recognitionRef.current?.abort();}},[]);
  useEffect(()=>{
  ...
  function stopContinuousVoice(){keepListeningRef.current=false;setVoiceAlwaysOn(false);if(recognitionRestart.current){clearTimeout(recognitionRestart.current);recognitionRestart.current=null;}try{recognitionRef.current?.stop();}catch{}recognitionRef.current=null;}
  function startContinuousVoice(){const Ctor=speechCtor();if(!Ctor){setVoiceSupported(false);notify('Continuous voice recognition is not supported by this browser or WebView. Text chat still works.');return;}setVoiceSupported(true);keepListeningRef.current=true;setVoiceAlwaysOn(true);const recognition=new Ctor();recognition.continuous=true;recognition.interimResults=true;recognition.lang=navigator.language||'en-US';recognition.onresult=(event)=>{if(speakingRef.current)return;let finalText='';let interim='';for(let i=0;i<event.results.length;i++){const result=event.results[i];const transcript=result[0]?.transcript?.trim()??'';if(result.isFinal)finalText+=(finalText?' ':'')+transcript;else interim+=(interim?' ':'')+transcript;}setLastHeard(finalText||interim);if(finalText)void sendMyria(undefined,finalText);};recognition.onerror=(event)=>{if(event.error==='not-allowed'||event.error==='service-not-allowed'){keepListeningRef.current=false;setVoiceAlwaysOn(false);notify('Microphone permission is required for always-listening mode.');}};recognition.onend=()=>{if(keepListeningRef.current){recognitionRestart.current=setTimeout(()=>{try{recognition.start();}catch{}},350);}};recognitionRef.current=recognition;try{recognition.start();notify('Always-listening mode is on. Myria will respond after you finish speaking.');}catch{stopContinuousVoice();notify('The microphone could not start.');}}
  function toggleContinuousVoice(){if(voiceAlwaysOn)stopContinuousVoice();else startContinuousVoice();}
