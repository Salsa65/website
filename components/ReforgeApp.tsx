...
export default function ReforgeApp(){
  ...
  const claimPendingInvite=useCallback(async()=>{if(!supabase||typeof window==='undefined')return;const token=window.localStorage.getItem('reforge-pending-invite');if(!token)return;const {data,error}=await supabase.rpc('accept_collaboration_invite',{p_token:token});if(error)return;window.localStorage.removeItem('reforge-pending-invite');if(data)window.history.replaceState(null,'',`${BASE_PATH}/?project=${encodeURIComponent(String(data))}`);notify('Private invite accepted automatically.');},[notify]);
